...
import '@archbase/layout/dist/layout.css'
...
import testFixesFinalContainerIOC from './ioc/TestFixesFinalContainerIOC'
import { TestFixesFinalDark, TestFixesFinalLight } from './theme'
import { navigationData } from './navigation/navigationData'
import { TestFixesFinalErrorFallback } from './utils/TestFixesFinalErrorFallback'
import { TestFixesFinalUser } from './auth/TestFixesFinalAuthenticator'
import { API_TYPE } from './ioc/TestFixesFinalIOCTypes'
// Do pacote core
import {
  useArchbaseTheme,
  processErrorMessage,
...
type MainProps = {
  onLoginUser: (user: TestFixesFinalUser) => void
  onLogoutUser: () => void
  user?: TestFixesFinalUser | undefined
  setUser: Dispatch<SetStateAction<TestFixesFinalUser | undefined>>
}
...
function Main({ onLoginUser, onLogoutUser, user, setUser }: MainProps) {
  ...
  const headerActions = () => {
    ...
    if (api && api.includes('localhost')) {
      result.push(
        <Badge size="lg" variant="gradient" gradient={{ from: '#2f3eeb', to: '#5b63f0', deg: 90 }}>
          {`${i18next.t('test-fixes-final:DESENVOLVIMENTO')}`}
        </Badge>
      )
    ...
    result.push(
      <Tooltip withinPortal withArrow label={`${i18next.t('test-fixes-final:Tela cheia')}`}>
    ...
    result.push(
      <Tooltip withinPortal withArrow label={`${i18next.t('test-fixes-final:Notificações')}`}>
  ...
  return (
    ...
    <ArchbaseErrorBoundary
      ...
      FallbackComponent={ TestFixesFinalErrorFallback}
      onError={logError}
    >
      {isAuthenticated && user ? (
        <Fragment>
          <ArchbaseAdminMainLayout
            ...
            sideBarBackgroundDarkColor={theme.colors.testFixesFinal[7]}
            sideBarBackgroundLightColor={theme.colors.testFixesFinal[7]}
            showHeader={isMedium}
            ...
            sideBarHeaderContent={
              !isMedium &&
              <Fragment>
                <Group py={4} pr={isCollapsed ? 0 : 16} bg={theme.colors.testFixesFinal[8]} wrap='nowrap' gap={8} justify={isCollapsed ? "center" : "space-between"}>
                  <Text 
                    style={{
                      fontSize: 20,
                      fontWeight: 'bold',
                  ...
                  >
                    {isCollapsed ? 'Test-fixes-final' : 'Test-fixes-final Admin'}
                  </Text>
            ...
            sideBarFooterContent={
              <Stack gap={4}>
                <Menu shadow="md" width={200} position="bottom-end" withArrow arrowPosition='center' offset={-5}>
                  <Menu.Target>
                    <Group gap={20} h={50} justify={isCollapsed ? 'center' : "flex-start"}>
                      ...
                      {api && api.includes('localhost') && !isCollapsed ?
                        ...
                        <Badge size="lg" variant="gradient" gradient={{ from: '#2f3eeb', to: '#5b63f0', deg: 90 }}>
                          {`${i18next.t('test-fixes-final:HOMOLOGAÇÃO')}`}
                        </Badge>
                      }
                    </Group>
                  </Menu.Target>
                  ...
                  <Menu.Dropdown >
                    <Menu.Label>{`${i18next.t('test-fixes-final:Usuário')}`}</Menu.Label>
                    <Menu.Item
                      leftSection={<IconUserCircle size={14} />}
                      onClick={handleOpenMyProfileModal}
                    >
                      {`${i18next.t('test-fixes-final:Meu Perfil')}`}
                    </Menu.Item>
                    ...
                    <Menu.Item leftSection={<IconArrowsMaximize size={14} />} onClick={toggle}>{`${i18next.t(
                      'test-fixes-final:Tela cheia'
                    )}`}</Menu.Item>
                    <Menu.Divider />
                    <Menu.Label>{`${i18next.t('test-fixes-final:Conta')}`}</Menu.Label>
                    <Menu.Item
                      leftSection={<IconBrandMessenger size={14} />}
                      onClick={toggleColorScheme}
                    >
                      {`${i18next.t('test-fixes-final:Suporte')}`}
                    </Menu.Item>
                    <Menu.Item
                      color="red"
                    ...
                    >
                      {`${i18next.t('test-fixes-final:Sair')}`}
                    </Menu.Item>
                  </Menu.Dropdown>
                </Menu>
                <Flex
                  ...
                  align="center"
                  style={{ paddingLeft: '8px', paddingRight: '8px', backgroundColor: theme.colors.testFixesFinal[8] }}
                  h={42}
                >
                  {!isCollapsed || isHidden ? 
                    <Text size="sm" c="white" fw={500}>Test-fixes-final</Text> 
                    : null
                  }
                  <Badge size='sm' mb={"8px"} color={theme.colors.testFixesFinal[5]}>
...
function App() {
  ...
  const [dark, setThemeDark] = useState<MantineThemeOverride>(TestFixesFinalDark)
  const [light, setThemeLight] = useState<MantineThemeOverride>(TestFixesFinalLight)
  const [currentUser, setCurrentUser] = useState<TestFixesFinalUser | undefined>(undefined)
  ...
  const handleLoginUser = (user: TestFixesFinalUser) => {
    setCurrentUser(user)
  }
  ...
  return (
    <ArchbaseGlobalProvider
      ...
      containerIOC={ testFixesFinalContainerIOC}
      themeDark={dark}
      themeLight={light}
      translationName="test-fixes-final"
      translationResource={{
        en: translation_en,
