...
import rapidexManagerAdminContainerIOC from './ioc/RapidexManagerAdminContainerIOC'
import { RapidexManagerAdminDark, RapidexManagerAdminLight } from './theme'
import { navigationData } from './navigation/navigationData'
import { RapidexManagerAdminErrorFallback } from './utils/RapidexManagerAdminErrorFallback'
import { RapidexManagerAdminUser } from './auth/RapidexManagerAdminAuthenticator'
import { API_TYPE } from './ioc/RapidexManagerAdminIOCTypes'
import {
...
type MainProps = {
  onLoginUser: (user: RapidexManagerAdminUser) => void
  onLogoutUser: () => void
  user?: RapidexManagerAdminUser | undefined
  setUser: Dispatch<SetStateAction<RapidexManagerAdminUser | undefined>>
}
...
function Main({ onLoginUser, onLogoutUser, user, setUser }: MainProps) {
  ...
  const headerActions = () => {
    ...
    if (api && api.includes('localhost')) {
      result.push(
        <Badge size="lg" variant="gradient" gradient={{ from: '#2f3eeb', to: '#5b63f0', deg: 90 }}>
          {`${i18next.t('rapidex-manager-admin:DESENVOLVIMENTO')}`}
        </Badge>
      )
    ...
    result.push(
      <Tooltip withinPortal withArrow label={`${i18next.t('rapidex-manager-admin:Tela cheia')}`}>
    ...
    result.push(
      <Tooltip withinPortal withArrow label={`${i18next.t('rapidex-manager-admin:Notificações')}`}>
  ...
  const handleUpdateUser = (newName: string, newAvatar?: string) => {
    setUser(prev => {
      ...
      return new RapidexManagerAdminUser({
        id: prev.id,
  ...
  return (
    ...
    <ArchbaseErrorBoundary
      ...
      FallbackComponent={ RapidexManagerAdminErrorFallback}
      onError={logError}
    >
      {isAuthenticated && user ? (
        <Fragment>
          <ArchbaseAdminMainLayout
            ...
            sideBarBackgroundDarkColor={theme.colors.rapidexManagerAdmin[7]}
            sideBarBackgroundLightColor={theme.colors.rapidexManagerAdmin[7]}
            showHeader={isMedium}
            ...
            sideBarHeaderContent={
              !isMedium &&
              <Fragment>
                <Group py={4} pr={isCollapsed ? 0 : 16} bg={theme.colors.rapidexManagerAdmin[8]} wrap='nowrap' gap={8} justify={isCollapsed ? "center" : "space-between"}>
                  <Text 
                    style={{
                      fontSize: 20,
                      fontWeight: 'bold',
                  ...
                  >
                    {isCollapsed ? 'Rapidex-manager-admin' : 'Rapidex-manager-admin Admin'}
                  </Text>
            ...
            sideBarFooterContent={
              <Stack gap={4}>
                <Menu shadow="md" width={200} position="bottom-end" withArrow arrowPosition='center' offset={-5}>
                  <Menu.Target>
                    <Group gap={20} h={50} justify={isCollapsed ? 'center' : "flex-start"}>
                      ...
                      {api && api.includes('localhost') && !isCollapsed ?
                        ...
                        <Badge size="lg" variant="gradient" gradient={{ from: '#2f3eeb', to: '#5b63f0', deg: 90 }}>
                          {`${i18next.t('rapidex-manager-admin:HOMOLOGAÇÃO')}`}
                        </Badge>
                      }
                    </Group>
                  </Menu.Target>
                  ...
                  <Menu.Dropdown >
                    <Menu.Label>{`${i18next.t('rapidex-manager-admin:Usuário')}`}</Menu.Label>
                    <Menu.Item
                      leftSection={<IconUserCircle size={14} />}
                      onClick={handleOpenMyProfileModal}
                    >
                      {`${i18next.t('rapidex-manager-admin:Meu Perfil')}`}
                    </Menu.Item>
                    ...
                    <Menu.Item leftSection={<IconArrowsMaximize size={14} />} onClick={toggle}>{`${i18next.t(
                      'rapidex-manager-admin:Tela cheia'
                    )}`}</Menu.Item>
                    <Menu.Divider />
                    <Menu.Label>{`${i18next.t('rapidex-manager-admin:Conta')}`}</Menu.Label>
                    <Menu.Item
                      leftSection={<IconBrandMessenger size={14} />}
                      onClick={toggleColorScheme}
                    >
                      {`${i18next.t('rapidex-manager-admin:Suporte')}`}
                    </Menu.Item>
                    <Menu.Item
                      color="red"
                    ...
                    >
                      {`${i18next.t('rapidex-manager-admin:Sair')}`}
                    </Menu.Item>
                  </Menu.Dropdown>
                </Menu>
                <Flex
                  ...
                  align="center"
                  style={{ paddingLeft: '8px', paddingRight: '8px', backgroundColor: theme.colors.rapidexManagerAdmin[8] }}
                  h={42}
                >
                  {!isCollapsed || isHidden ? 
                    <Text size="sm" c="white" fw={500}>Rapidex-manager-admin</Text> 
                    : null
                  }
                  <Badge size='sm' mb={"8px"} color={theme.colors.rapidexManagerAdmin[5]}>
...
function App() {
  ...
  const [dark, setThemeDark] = useState<MantineThemeOverride>(RapidexManagerAdminDark)
  const [light, setThemeLight] = useState<MantineThemeOverride>(RapidexManagerAdminLight)
  const [currentUser, setCurrentUser] = useState<RapidexManagerAdminUser | undefined>(undefined)
  ...
  const handleLoginUser = (user: RapidexManagerAdminUser) => {
    setCurrentUser(user)
  }
  ...
  return (
    <ArchbaseGlobalProvider
      ...
      containerIOC={ rapidexManagerAdminContainerIOC}
      themeDark={dark}
      themeLight={light}
      translationName="rapidex-manager-admin"
      translationResource={{
        en: translation_en,
