...
import testPostcssFixContainerIOC from './ioc/TestPostcssFixContainerIOC'
import { TestPostcssFixDark, TestPostcssFixLight } from './theme'
import { navigationData } from './navigation/navigationData'
import { TestPostcssFixErrorFallback } from './utils/TestPostcssFixErrorFallback'
import { TestPostcssFixUser } from './auth/TestPostcssFixAuthenticator'
import { API_TYPE } from './ioc/TestPostcssFixIOCTypes'
import {
...
type MainProps = {
  onLoginUser: (user: TestPostcssFixUser) => void
  onLogoutUser: () => void
  user?: TestPostcssFixUser | undefined
  setUser: Dispatch<SetStateAction<TestPostcssFixUser | undefined>>
}
...
function Main({ onLoginUser, onLogoutUser, user, setUser }: MainProps) {
  ...
  const headerActions = () => {
    ...
    if (api && api.includes('localhost')) {
      result.push(
        <Badge size="lg" variant="gradient" gradient={{ from: '#2f3eeb', to: '#5b63f0', deg: 90 }}>
          {`${i18next.t('test-postcss-fix:DESENVOLVIMENTO')}`}
        </Badge>
      )
    ...
    result.push(
      <Tooltip withinPortal withArrow label={`${i18next.t('test-postcss-fix:Tela cheia')}`}>
    ...
    result.push(
      <Tooltip withinPortal withArrow label={`${i18next.t('test-postcss-fix:Notificações')}`}>
  ...
  return (
    ...
    <ArchbaseErrorBoundary
      ...
      FallbackComponent={ TestPostcssFixErrorFallback}
      onError={logError}
    >
      {isAuthenticated && user ? (
        <Fragment>
          <ArchbaseAdminMainLayout
            ...
            sideBarBackgroundDarkColor={theme.colors.testPostcssFix[7]}
            sideBarBackgroundLightColor={theme.colors.testPostcssFix[7]}
            showHeader={isMedium}
            ...
            sideBarHeaderContent={
              !isMedium &&
              <Fragment>
                <Group py={4} pr={isCollapsed ? 0 : 16} bg={theme.colors.testPostcssFix[8]} wrap='nowrap' gap={8} justify={isCollapsed ? "center" : "space-between"}>
                  <Text 
                    style={{
                      fontSize: 20,
                      fontWeight: 'bold',
                  ...
                  >
                    {isCollapsed ? 'Test-postcss-fix' : 'Test-postcss-fix Admin'}
                  </Text>
            ...
            sideBarFooterContent={
              <Stack gap={4}>
                <Menu shadow="md" width={200} position="bottom-end" withArrow arrowPosition='center' offset={-5}>
                  <Menu.Target>
                    <Group gap={20} h={50} justify={isCollapsed ? 'center' : "flex-start"}>
                      ...
                      {api && api.includes('localhost') && !isCollapsed ?
                        ...
                        <Badge size="lg" variant="gradient" gradient={{ from: '#2f3eeb', to: '#5b63f0', deg: 90 }}>
                          {`${i18next.t('test-postcss-fix:HOMOLOGAÇÃO')}`}
                        </Badge>
                      }
                    </Group>
                  </Menu.Target>
                  ...
                  <Menu.Dropdown >
                    <Menu.Label>{`${i18next.t('test-postcss-fix:Usuário')}`}</Menu.Label>
                    <Menu.Item
                      leftSection={<IconUserCircle size={14} />}
                      onClick={handleOpenMyProfileModal}
                    >
                      {`${i18next.t('test-postcss-fix:Meu Perfil')}`}
                    </Menu.Item>
                    ...
                    <Menu.Item leftSection={<IconArrowsMaximize size={14} />} onClick={toggle}>{`${i18next.t(
                      'test-postcss-fix:Tela cheia'
                    )}`}</Menu.Item>
                    <Menu.Divider />
                    <Menu.Label>{`${i18next.t('test-postcss-fix:Conta')}`}</Menu.Label>
                    <Menu.Item
                      leftSection={<IconBrandMessenger size={14} />}
                      onClick={toggleColorScheme}
                    >
                      {`${i18next.t('test-postcss-fix:Suporte')}`}
                    </Menu.Item>
                    <Menu.Item
                      color="red"
                    ...
                    >
                      {`${i18next.t('test-postcss-fix:Sair')}`}
                    </Menu.Item>
                  </Menu.Dropdown>
                </Menu>
                <Flex
                  ...
                  align="center"
                  style={{ paddingLeft: '8px', paddingRight: '8px', backgroundColor: theme.colors.testPostcssFix[8] }}
                  h={42}
                >
                  {!isCollapsed || isHidden ? 
                    <Text size="sm" c="white" fw={500}>Test-postcss-fix</Text> 
                    : null
                  }
                  <Badge size='sm' mb={"8px"} color={theme.colors.testPostcssFix[5]}>
...
function App() {
  ...
  const [dark, setThemeDark] = useState<MantineThemeOverride>(TestPostcssFixDark)
  const [light, setThemeLight] = useState<MantineThemeOverride>(TestPostcssFixLight)
  const [currentUser, setCurrentUser] = useState<TestPostcssFixUser | undefined>(undefined)
  ...
  const handleLoginUser = (user: TestPostcssFixUser) => {
    setCurrentUser(user)
  }
  ...
  return (
    <ArchbaseGlobalProvider
      ...
      containerIOC={ testPostcssFixContainerIOC}
      themeDark={dark}
      themeLight={light}
      translationName="test-postcss-fix"
      translationResource={{
        en: translation_en,
