...
export function HomeView() {
  return (
    <Container size="lg" py="xl">
      <Group mb="xl">
        <IconHome size={32} />
        <Title order={1}>{i18next.t('test-postcss-fix:Home')}</Title>
      </Group>
      
      <Stack gap="md">
        <Card shadow="sm" padding="lg" radius="md" withBorder>
          <Title order={2} mb="md">
            {i18next.t('test-postcss-fix:Bem-vindo ao Test-postcss-fix')}
          </Title>
          <Text c="dimmed">
            {i18next.t('test-postcss-fix:Esta é sua página inicial')}
          </Text>
        </Card>
