...
import testSecurityAppContainerIOC from './ioc/TestSecurityAppContainerIOC'
import { TestSecurityAppDark, TestSecurityAppLight } from './theme'
import { navigationData } from './navigation/navigationData'
import { TestSecurityAppErrorFallback } from './utils/TestSecurityAppErrorFallback'
import { TestSecurityAppUser } from './auth/TestSecurityAppAuthenticator'
import { API_TYPE } from './ioc/TestSecurityAppIOCTypes'
import {
...
type MainProps = {
  onLoginUser: (user: TestSecurityAppUser) => void
  onLogoutUser: () => void
  user?: TestSecurityAppUser | undefined
  setUser: Dispatch<SetStateAction<TestSecurityAppUser | undefined>>
}
...
function Main({ onLoginUser, onLogoutUser, user, setUser }: MainProps) {
  ...
  const headerActions = () => {
    ...
    if (api && api.includes('localhost')) {
      result.push(
        <Badge size="lg" variant="gradient" gradient={{ from: '#2f3eeb', to: '#5b63f0', deg: 90 }}>
          {`${i18next.t('test-security-app:DESENVOLVIMENTO')}`}
        </Badge>
      )
    ...
    result.push(
      <Tooltip withinPortal withArrow label={`${i18next.t('test-security-app:Tela cheia')}`}>
    ...
    result.push(
      <Tooltip withinPortal withArrow label={`${i18next.t('test-security-app:Notificações')}`}>
  ...
  const handleUpdateUser = (newName: string, newAvatar?: string) => {
    setUser(prev => {
      ...
      return new TestSecurityAppUser({
        id: prev.id,
  ...
  return (
    ...
    <ArchbaseErrorBoundary
      ...
      FallbackComponent={ TestSecurityAppErrorFallback}
      onError={logError}
    >
      {isAuthenticated && user ? (
        <Fragment>
          <ArchbaseAdminMainLayout
            ...
            sideBarBackgroundDarkColor={theme.colors.testSecurityApp[7]}
            sideBarBackgroundLightColor={theme.colors.testSecurityApp[7]}
            showHeader={isMedium}
            ...
            sideBarHeaderContent={
              !isMedium &&
              <Fragment>
                <Group py={4} pr={isCollapsed ? 0 : 16} bg={theme.colors.testSecurityApp[8]} wrap='nowrap' gap={8} justify={isCollapsed ? "center" : "space-between"}>
                  <Text 
                    style={{
                      fontSize: 20,
                      fontWeight: 'bold',
                  ...
                  >
                    {isCollapsed ? 'Test-security-app' : 'Test-security-app Admin'}
                  </Text>
            ...
            sideBarFooterContent={
              <Stack gap={4}>
                <Menu shadow="md" width={200} position="bottom-end" withArrow arrowPosition='center' offset={-5}>
                  <Menu.Target>
                    <Group gap={20} h={50} justify={isCollapsed ? 'center' : "flex-start"}>
                      ...
                      {api && api.includes('localhost') && !isCollapsed ?
                        ...
                        <Badge size="lg" variant="gradient" gradient={{ from: '#2f3eeb', to: '#5b63f0', deg: 90 }}>
                          {`${i18next.t('test-security-app:HOMOLOGAÇÃO')}`}
                        </Badge>
                      }
                    </Group>
                  </Menu.Target>
                  ...
                  <Menu.Dropdown >
                    <Menu.Label>{`${i18next.t('test-security-app:Usuário')}`}</Menu.Label>
                    <Menu.Item
                      leftSection={<IconUserCircle size={14} />}
                      onClick={handleOpenMyProfileModal}
                    >
                      {`${i18next.t('test-security-app:Meu Perfil')}`}
                    </Menu.Item>
                    ...
                    <Menu.Item leftSection={<IconArrowsMaximize size={14} />} onClick={toggle}>{`${i18next.t(
                      'test-security-app:Tela cheia'
                    )}`}</Menu.Item>
                    <Menu.Divider />
                    <Menu.Label>{`${i18next.t('test-security-app:Conta')}`}</Menu.Label>
                    <Menu.Item
                      leftSection={<IconBrandMessenger size={14} />}
                      onClick={toggleColorScheme}
                    >
                      {`${i18next.t('test-security-app:Suporte')}`}
                    </Menu.Item>
                    <Menu.Item
                      color="red"
                    ...
                    >
                      {`${i18next.t('test-security-app:Sair')}`}
                    </Menu.Item>
                  </Menu.Dropdown>
                </Menu>
                <Flex
                  ...
                  align="center"
                  style={{ paddingLeft: '8px', paddingRight: '8px', backgroundColor: theme.colors.testSecurityApp[8] }}
                  h={42}
                >
                  {!isCollapsed || isHidden ? 
                    <Text size="sm" c="white" fw={500}>Test-security-app</Text> 
                    : null
                  }
                  <Badge size='sm' mb={"8px"} color={theme.colors.testSecurityApp[5]}>
...
function App() {
  ...
  const [dark, setThemeDark] = useState<MantineThemeOverride>(TestSecurityAppDark)
  const [light, setThemeLight] = useState<MantineThemeOverride>(TestSecurityAppLight)
  const [currentUser, setCurrentUser] = useState<TestSecurityAppUser | undefined>(undefined)
  ...
  const handleLoginUser = (user: TestSecurityAppUser) => {
    setCurrentUser(user)
  }
  ...
  return (
    <ArchbaseGlobalProvider
      ...
      containerIOC={ testSecurityAppContainerIOC}
      themeDark={dark}
      themeLight={light}
      translationName="test-security-app"
      translationResource={{
        en: translation_en,
