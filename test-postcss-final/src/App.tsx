...
import testPostcssFinalContainerIOC from './ioc/TestPostcssFinalContainerIOC'
import { TestPostcssFinalDark, TestPostcssFinalLight } from './theme'
import { navigationData } from './navigation/navigationData'
import { TestPostcssFinalErrorFallback } from './utils/TestPostcssFinalErrorFallback'
import { TestPostcssFinalUser } from './auth/TestPostcssFinalAuthenticator'
import { API_TYPE } from './ioc/TestPostcssFinalIOCTypes'
import {
...
type MainProps = {
  onLoginUser: (user: TestPostcssFinalUser) => void
  onLogoutUser: () => void
  user?: TestPostcssFinalUser | undefined
  setUser: Dispatch<SetStateAction<TestPostcssFinalUser | undefined>>
}
...
function Main({ onLoginUser, onLogoutUser, user, setUser }: MainProps) {
  ...
  const headerActions = () => {
    ...
    if (api && api.includes('localhost')) {
      result.push(
        <Badge size="lg" variant="gradient" gradient={{ from: '#2f3eeb', to: '#5b63f0', deg: 90 }}>
          {`${i18next.t('test-postcss-final:DESENVOLVIMENTO')}`}
        </Badge>
      )
    ...
    result.push(
      <Tooltip withinPortal withArrow label={`${i18next.t('test-postcss-final:Tela cheia')}`}>
    ...
    result.push(
      <Tooltip withinPortal withArrow label={`${i18next.t('test-postcss-final:Notificações')}`}>
  ...
  const handleUpdateUser = (newName: string, newAvatar?: string) => {
    setUser(prev => {
      ...
      return new TestPostcssFinalUser({
        id: prev.id,
  ...
  return (
    ...
    <ArchbaseErrorBoundary
      ...
      FallbackComponent={ TestPostcssFinalErrorFallback}
      onError={logError}
    >
      {isAuthenticated && user ? (
        <Fragment>
          <ArchbaseAdminMainLayout
            ...
            sideBarBackgroundDarkColor={theme.colors.testPostcssFinal[7]}
            sideBarBackgroundLightColor={theme.colors.testPostcssFinal[7]}
            showHeader={isMedium}
            ...
            sideBarHeaderContent={
              !isMedium &&
              <Fragment>
                <Group py={4} pr={isCollapsed ? 0 : 16} bg={theme.colors.testPostcssFinal[8]} wrap='nowrap' gap={8} justify={isCollapsed ? "center" : "space-between"}>
                  <Text 
                    style={{
                      fontSize: 20,
                      fontWeight: 'bold',
                  ...
                  >
                    {isCollapsed ? 'Test-postcss-final' : 'Test-postcss-final Admin'}
                  </Text>
            ...
            sideBarFooterContent={
              <Stack gap={4}>
                <Menu shadow="md" width={200} position="bottom-end" withArrow arrowPosition='center' offset={-5}>
                  <Menu.Target>
                    <Group gap={20} h={50} justify={isCollapsed ? 'center' : "flex-start"}>
                      ...
                      {api && api.includes('localhost') && !isCollapsed ?
                        ...
                        <Badge size="lg" variant="gradient" gradient={{ from: '#2f3eeb', to: '#5b63f0', deg: 90 }}>
                          {`${i18next.t('test-postcss-final:HOMOLOGAÇÃO')}`}
                        </Badge>
                      }
                    </Group>
                  </Menu.Target>
                  ...
                  <Menu.Dropdown >
                    <Menu.Label>{`${i18next.t('test-postcss-final:Usuário')}`}</Menu.Label>
                    <Menu.Item
                      leftSection={<IconUserCircle size={14} />}
                      onClick={handleOpenMyProfileModal}
                    >
                      {`${i18next.t('test-postcss-final:Meu Perfil')}`}
                    </Menu.Item>
                    ...
                    <Menu.Item leftSection={<IconArrowsMaximize size={14} />} onClick={toggle}>{`${i18next.t(
                      'test-postcss-final:Tela cheia'
                    )}`}</Menu.Item>
                    <Menu.Divider />
                    <Menu.Label>{`${i18next.t('test-postcss-final:Conta')}`}</Menu.Label>
                    <Menu.Item
                      leftSection={<IconBrandMessenger size={14} />}
                      onClick={toggleColorScheme}
                    >
                      {`${i18next.t('test-postcss-final:Suporte')}`}
                    </Menu.Item>
                    <Menu.Item
                      color="red"
                    ...
                    >
                      {`${i18next.t('test-postcss-final:Sair')}`}
                    </Menu.Item>
                  </Menu.Dropdown>
                </Menu>
                <Flex
                  ...
                  align="center"
                  style={{ paddingLeft: '8px', paddingRight: '8px', backgroundColor: theme.colors.testPostcssFinal[8] }}
                  h={42}
                >
                  {!isCollapsed || isHidden ? 
                    <Text size="sm" c="white" fw={500}>Test-postcss-final</Text> 
                    : null
                  }
                  <Badge size='sm' mb={"8px"} color={theme.colors.testPostcssFinal[5]}>
...
function App() {
  ...
  const [dark, setThemeDark] = useState<MantineThemeOverride>(TestPostcssFinalDark)
  const [light, setThemeLight] = useState<MantineThemeOverride>(TestPostcssFinalLight)
  const [currentUser, setCurrentUser] = useState<TestPostcssFinalUser | undefined>(undefined)
  ...
  const handleLoginUser = (user: TestPostcssFinalUser) => {
    setCurrentUser(user)
  }
  ...
  return (
    <ArchbaseGlobalProvider
      ...
      containerIOC={ testPostcssFinalContainerIOC}
      themeDark={dark}
      themeLight={light}
      translationName="test-postcss-final"
      translationResource={{
        en: translation_en,
