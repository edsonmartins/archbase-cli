...
export class KnowledgeBase {
  ...
  async searchPatterns(description: string, category?: string): Promise<PatternInfo[]> {
  ...
  async searchExamples(filters: { component?: string; pattern?: string; tag?: string }): Promise<ExampleInfo[]> {
    ...
    for (const component of this.componentsCache.values()) {
      const filtered = component.examples.filter(example => {
        if (filters.component && component.name !== filters.component) return false;
        if (filters.tag && !example.tags.includes(filters.tag)) return false;
        return true;
      });
  ...
  async freeSearch(query: string): Promise<{
  ...
  private async loadPatterns(): Promise<void> {
    try {
      ...
      if (await fs.pathExists(patternsFile)) {
        ...
        if (data.patterns) {
          this.patternsCache = Object.values(data.patterns) as PatternInfo[];
        }
      } else {
